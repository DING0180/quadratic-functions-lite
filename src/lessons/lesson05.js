...
function updateGraph(graph, parameters, options = {}) {
  ...
  graph.update({
    ...
    points: valid ? [{ x: parameters.h, y: parameters.k, color: COLORS.k, radius: 5 }] : [],
...
function renderProperties(root) {
  ...
  ].map((copy) => { const row = element("p", "lesson05-property", copy); row.hidden = true; return row; });
  ...
  root.append(element("p", "lesson05-prompt", "先根据实验读图，再逐条揭示性质。"), ...rows, reveal);
}
...
function renderShiftChallenge(root, _onStepChange, cleanup, random) {
  ...
  next.addEventListener("click", update); root.append(prompt, answer, element("div", "lesson05-actions"), graphPanel); root.querySelector(".lesson05-actions").append(reveal, movement, next); update();
}
